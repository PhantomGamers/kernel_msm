...
/* Resources */
int s2w_switch = S2W_DEFAULT, s2w_s2s = S2W_S2S_DEFAULT;
...
static void detect_sweep2wake(int x, int y, bool st)
{
	...
	//right->left
	} else if ((single_touch) && (scr_suspended == false) && (s2w_s2s > 0)) {
		scr_on_touch=true;
...
static ssize_t s2w_s2w_s2s_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t count = 0;
...
static ssize_t s2w_s2w_s2s_dump(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] >= '0' && buf[0] <= '1' && buf[1] == '\n')
                if (s2w_s2s != buf[0] - '0')
...
static DEVICE_ATTR(s2w_s2s, (S_IWUSR|S_IRUGO),
	s2w_s2w_s2s_show, s2w_s2w_s2s_dump);
...
static int __init sweep2wake_init(void)
{
	...
	rc = sysfs_create_file(android_touch_kobj, &dev_attr_s2w_s2s.attr);
	if (rc) {
		pr_warn("%s: sysfs_create_file failed for s2w_s2s\n", __func__);
	}
